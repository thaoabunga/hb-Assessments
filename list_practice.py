...
def average(numbers):
    ...
    for number in numbers:
        average_numbers = float(sum(numbers))/len(numbers)
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    For example::
        ...
        ['I', 'love', 'cookies']
    """
    size = len(items) #find the length of the list items
    for i in range(0, size/2): #for i in the
        items[i], items[size-1-i] = items[size-1-i], items[i]
